...
then
    ...
    kind create cluster --config ./deploy-cluster-kind/kind-cluster.yaml --name newyear
    ...
    if [ -f "./deploy-cluster-kind/recommended.yaml"]
    then
        rm ./deploy-cluster-kind/recommended.yaml
    fi
    # retrieve the yaml file with the proper version 
    curl -Lo ./deploy-cluster-kind/recommended.yaml https://raw.githubusercontent.com/kubernetes/dashboard/$VERSION/aio/deploy/recommended.yaml

    kubectl apply -f ./deploy-cluster-kind/recommended.yaml
    ...
    kubectl apply -f ./deploy-cluster-kind/dashboard-adminuser.yaml
