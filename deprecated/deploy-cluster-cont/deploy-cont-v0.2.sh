...
# kind v0.8.1 is running on Kubernetes v1.18.2, so you need to:
#   - install a compatible version of kubectl (1.18.2)
#       https://storage.googleapis.com/kubernetes-release/release/v1.18.2/bin/linux/amd64/kubectl
#   - copy the dashboard YAML file v2.0.0 which is compatible with Kubernetes v1.18:
...
cp kind-cluster-v0.2.yaml $sandbox_directory/$cluster_configuration
cp dashboard-v200-recommended.yaml $sandbox_directory/$dashboard_configuration
cp dashboard-adminuser.yaml $sandbox_directory/$dasbboard_user
